...
class ReminderController extends Controller
{
	...
	public function getRemind(Request $request)
	{
        ...
        return view('front.reminder.remind', $data);
	}
	...
	public function postRemind(Request $request)
	{
		...
		if (!$validator->fails())
		{
			$admin = User::where('email', '=', $request->input('email'))->where('is_active', '=', true)->where('is_suspended', '=', false)->first();
			if ($admin != null)
			{
	...
	public function postReset(Request $request, $token = null)
	{
		...
		else
		{
			...
			if ($validator->passes())
			{
				...
				{
					$admin = User::where('email', '=', $request->input('old_email'))->first();
					if($admin != null)
					{
